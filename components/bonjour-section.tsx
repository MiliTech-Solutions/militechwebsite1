...
export function BonjourSection({ title, subtitle, description, imageUrl, imageAlt }: BonjourProps) {
  ...
  return (
    <div ref={ref} className="mb-20 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
      <div className="relative">
        <div className={`absolute -top-4 left-0 w-1 h-20 bg-gradient-to-b from-orange-400 to-orange-600 shadow-[0_0_15px_rgba(249,115,22,0.5)] ${isIntersecting ? 'services-slide-in' : 'opacity-0'}`} />

        <div className="pl-8">
          <h1 className={`text-6xl md:text-7xl font-bold text-white mb-4 drop-shadow-[0_0_30px_rgba(255,255,255,0.1)] ${isIntersecting ? 'services-slide-in' : 'opacity-0'}`}>
            {title}
          </h1>
          <p className={`text-orange-400 text-xl mb-2 ${isIntersecting ? 'services-slide-in' : 'opacity-0'}`}>{subtitle}</p>
          <div className={`text-gray-300 text-lg mb-8 ${isIntersecting ? 'services-slide-in' : 'opacity-0'}`}>{description}</div>

          <div className={`w-32 h-1 bg-gradient-to-r from-orange-600 via-orange-400 to-orange-500 mb-8 shadow-[0_0_12px_rgba(249,115,22,0.5)] ${isIntersecting ? 'services-slide-in' : 'opacity-0'}`} />

          <div className={isIntersecting ? 'services-slide-in' : 'opacity-0'}>
            <Button
              asChild
          ...
          {/* Social Icons */}
          <div className={`flex gap-4 mt-8 ${isIntersecting ? 'services-slide-in' : 'opacity-0'}`}>
      ...
      <div className={`relative ${isIntersecting ? 'services-slide-in' : 'opacity-0'}`}>
